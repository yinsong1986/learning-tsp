...
class AttentionModel(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        if self.is_vrp or self.is_orienteering or self.is_pctsp:
            ...
            assert problem.NAME in ("tsp", "tspsl"), "Unsupported problem: {}".format(
                problem.NAME
            )
    ...
    def forward(
        ...
        if self.problem.NAME == "tspsl" and supervised:
            assert (
                targets is not None
            ), "Pass targets during training in supervised mode"

            # Run inner function
            _log_p, pi = self._inner(
                nodes, graph, embeddings, supervised=supervised, targets=targets
            )
            ...
            # Compute NLL loss
            logits = _log_p.permute(
                0, 2, 1
            )  # B x V x output_vocab -> B x output_vocab x V
    ...
    def _get_log_p(self, fixed, state, normalize=True):
        ...
        log_p, glimpse = self._one_to_many_logits(
            query, glimpse_K, glimpse_V, logit_K, mask, graph_mask
        )
    ...
    def _one_to_many_logits(
        self, query, glimpse_K, glimpse_V, logit_K, mask, graph_mask=None
    ):
        ...
        if self.mask_inner:
            ...
            if self.mask_graph:
                compatibility[
                    graph_mask[None, :, :, None, :].expand_as(compatibility)
                ] = -1e10
