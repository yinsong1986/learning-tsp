...
def validate(model, dataset, problem, opts):
    ...
    print(
        "Validation groundtruth cost: {:.3f} +- {:.3f}".format(
...
def rollout(model, dataset, opts):
    ...
    def eval_model_bat(bat):
        with torch.no_grad():
            cost, _ = model(
                move_to(bat["nodes"], opts.device), move_to(bat["graph"], opts.device)
            )
...
def train_batch(
    ...
    with torch.autograd.profiler.emit_nvtx():
        ...
        bl_val, bl_loss = (
            baseline.eval(x, graph, cost) if bl_val is None else (bl_val, 0)
        )
...
def train_epoch_sl(
    ...
    for batch_id, batch in enumerate(
        ...
        train_batch_sl(model, optimizer, epoch, batch_id, step, batch, tb_logger, opts)
...
def train_batch_sl(model, optimizer, epoch, batch_id, step, batch, tb_logger, opts):
    # Optionally move Tensors to GPU
    ...
    if opts.model == "nar":
        ...
        class_weights = compute_class_weight(
            "balanced", classes=np.unique(_targets), y=_targets
        )
    ...
    cost, loss = model(
        x, graph, supervised=True, targets=targets, class_weights=class_weights
    )
    ...
    # Logging
    if step % int(opts.log_step) == 0:
        log_values_sl(cost, grad_norms, epoch, batch_id, step, loss, tb_logger, opts)
